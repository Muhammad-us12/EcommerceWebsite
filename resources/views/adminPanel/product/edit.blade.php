                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('product.update', $product->id) }}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="row mb-2">
                            <div class="col-sm-12">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="user_id" id="example-select">
                                        ...
                                        <option value="{{ $vendor->id }}" {{ $product->user_id == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('user_id')
                            ...
                            <div class="col-sm-2">
                                <div class="mb-3">
                                    ...
                                    <select name="status" class="form-control" id="">
                                        <option value="{{ App\Enums\Status::ACTIVE->value }}" {{ $product->status == App\Enums\Status::ACTIVE->value  ? 'selected' : '' }}>Active</option>
                                        ...
                                        <option value="{{ App\Enums\Status::IN_REVIEW->value }}" {{ $product->status == App\Enums\Status::IN_REVIEW->value  ? 'selected' : '' }}>In Review</option>
                                        <option value="{{ App\Enums\Status::REJECTED->value }}" {{ $product->status == App\Enums\Status::REJECTED->value  ? 'selected' : '' }}>Rejected</option>
                                    </select>
                                    @error('cost_price')
                                    <p class="text-danger mt-2">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="col-sm-12">
                                <div class="mb-3">
                                    <label for="example-input-normal" class="form-label">Remarks (If You want to Reject)</label>
                                    <textarea name="review_remarks" class="form-control" id="">{{ $product->review_remarks }}</textarea>
                                </div>
                            </div>
                                ...
                                <div class="table-responsive">
                                    <table id="" class="table table-centered w-100 nowrap">
                                        ...
                                        <tbody id="extra_prices_table">
                                            ...
                                            @foreach($product->productExtraPrices as $extraPrice)
                                            <tr id="{{ $extraPrice->price_id }}">
                                                <td>
                                                    <h5>{{ $extraPrice->extraPrice->name }}</h5>
                                                    <input type="text" hidden name="price_ids[]" required class="form-control" value="{{ $extraPrice->price_id }}">
                                                </td>
                                                <td>
                                                    <input type="number" step="any" name="price_values[]" id="extraPrices{{ $extraPrice->id }}" value="{{ $extraPrice->value }}" required class="form-control">
                                                </td>
                                                <td>
                                                    <button class="btn btn-danger btn-sm" type="button" onclick="removeExtraPrice({{ $extraPrice->price_id }})">X</button>
                                                </td>   
                                            </tr>
                                            <script>
                                                addToCartExtraPricesArr.push({{ $extraPrice->price_id }});
                                            </script>
                                            @endforeach
                                            @endisset
                                        </tbody>
